...
import { Home, Bath, Maximize, Building2, Check, Star, Mail, Heart, MapPin, Calendar, CheckCircle } from "lucide-react";
...
const reviews = [
  { name: "Michael Chen", role: "Buyer", rating: 5, text: "Exceptional property and seamless transaction. The listing was accurate and the agent was incredibly helpful throughout the process." },
  { name: "Jessica Williams", role: "Tenant", rating: 4, text: "Great experience with DreamNest. The property exceeded expectations and the rental process was straightforward and professional." },
  { name: "Robert Kim", role: "Investor", rating: 5, text: "Outstanding investment opportunity. The property has appreciated significantly and the rental yield has been excellent." },
];

const amenities = ["Central AC", "Hardwood Floors", "Stainless Appliances", "Walk-in Closets", "Pool Access", "Fitness Center", "Parking Garage", "Rooftop Terrace"];
...
export default function PropertyDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await api.get<PropertyDetail>(`/api/properties/${params.id}`);
        setProperty(res);
        setSelectedImage(0);
        ...
        setRelated(relatedRes.properties.filter((p) => (p.id || p._id) !== params.id).slice(0, 4));
      } catch {
        setProperty(null);
      } finally {
        setLoading(false);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-base-200">
        <div className="text-center">
          ...
          <p className="text-gray mb-6">The property you&apos;re looking for doesn&apos;t exist.</p>
          <Link href="/explore">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card bg-white rounded-box border border-gray-light p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {([
                  ...
                  { label: "Type", value: property.category.charAt(0).toUpperCase() + property.category.slice(1), Icon: Building2 },
                ] as unknown as { label: string; value: string | number; Icon: React.ComponentType<{ className?: string }> }[]).map((item) => (
                  <div key={item.label} className="flex items-center gap-3 p-3 rounded-field bg-base-200">
            ...
            {property.fullDescription && (
              <div className="card bg-white rounded-box border border-gray-light p-6">
                ...
                <p className="text-gray leading-relaxed whitespace-pre-line">{property.fullDescription}</p>
              </div>
            )}
            ...
            <div className="card bg-white rounded-box border border-gray-light p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {amenities.map((a) => (
                  <div key={a} className="flex items-center gap-2 text-sm text-gray">
            ...
            <div className="card bg-white rounded-box border border-gray-light p-6">
              ...
              <div className="space-y-4">
                {reviews.map((review) => (
                  <div key={review.name} className="border-b border-gray-light pb-4 last:border-0 last:pb-0">
        ...
        {related.length > 0 && (
          <motion.div
            ...
            className="mt-16"
          >
            <h2 className="font-heading text-2xl md:text-3xl text-navy mb-8">Related Properties</h2>
